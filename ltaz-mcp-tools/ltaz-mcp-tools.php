<?php
/**
 * Plugin Name: LTAZ MCP Tools
 * Plugin URI:  https://github.com/pdevvle/ltaz
 * Description: Companion plugin for AI Engine that brings this site's MCP tool surface to parity with the PPS staging server: navigation menus, theme mods, Astra settings, Customizer CSS, reusable blocks, widget areas, site identity, plugin/theme file management, uploads cleanup and retention, WordPress updates, and (when WooCommerce is active) store tools.
 * Version:     1.0.0
 * Author:      Lee's Trees
 ...
 *
 * WHAT THIS IS
 *  A port of the Priority Print MCP Tools plugin (v1.8.0) that runs on the PPS
 *  staging site, extended with the 20 site-configuration tools that server also
 *  exposes, and adapted for a site that is not Priority Print.
 *
 *  Tools register through AI Engine's two MCP filters -- mwai_mcp_tools (advertise)
 *  and mwai_mcp_callback (execute). AI Engine's MCP module must be enabled, and its
 *  bearer-token auth is what protects every tool here.
 *
 * TOOL NAMES
 *  Tools keep the pps_ prefix by default so this surface is a literal drop-in match
 *  for the staging server -- prompts and automations written against PPS work here
 *  unchanged. Define LTAZ_MCP_PREFIX in wp-config.php to rebrand, e.g.
 *      define( 'LTAZ_MCP_PREFIX', 'ltaz_' );
 *
 * DIFFERENCES FROM THE PPS ORIGINAL (all deliberate)
 *  - Theme writes target the ACTIVE stylesheet (the child theme when one is active)
 *    rather than a hard-coded slug. Override with LTAZ_MCP_THEME_SLUG. Note that
 *    writing into a parent theme you did not author means the next theme update
 *    overwrites your edits -- use a child theme.
 *  - The woo_* group is only advertised when WooCommerce is active, and the astra_*
 *    group only when Astra is present. A tool that could only ever return
 *    "not active" is worse than an absent tool.
 *  - Theme and plugin path containment now use the same strict test the uploads
 *    helper already used (exact match or trailing separator). The original's
 *    strpos( $check, $root ) !== 0 prefix test would also accept a sibling
 *    directory that merely shares the prefix, e.g. wp-content/plugins-old.
 *  - Retention option / cron keys are namespaced ltaz_* so they cannot collide
 *    with a PPS install.
 *
 * SAFETY MODEL (inherited from the original)
 *  - Theme writes are restricted to the one allowed theme directory
 *  - Plugin writes (including downloads and deletes) are restricted to wp-content/plugins
 *  - Path traversal is blocked via realpath() validation
 *  - plugin_download_url enforces https:// only, 12MB max, 60s timeout
 *  - plugin_delete_file removes a single file only: refuses directories, this
 *    plugin's own file, and any active plugin's entry file
 *  - Uploads tools are restricted to wp_upload_dir()['basedir']
 *  - uploads_delete_file / uploads_delete_batch take explicit paths only -- never a
 *    pattern or glob. Both refuse directories, the directory-guard files
 *    (index.php / index.html / .htaccess / web.config), and any file still backing a
 *    media-library attachment. Batch is capped at 500 paths and reports each
 *    per-file failure rather than aborting
 *  - Uploads retention ships OFF, with dry_run ON and no directory, so it is inert
 *    until deliberately configured. It refuses the uploads root, floors
 *    min_age_days at RETENTION_MIN_AGE_FLOOR, caps deletes per run, and logs every
 *    run to ltaz_uploads_retention_log
 *  - Every write / download / delete calls opcache_invalidate() on the affected file
 ...
 *
 * BEHAVIOUR-CARRYING OPTIONS
 *  ltaz_uploads_retention (policy) and ltaz_uploads_retention_log (history).
 */
...
class LTAZ_MCP_Tools {

	/**
	 * Tool-name prefix. Defaults to "pps_" so the surface is a literal drop-in
	 * match for the PPS staging server and any prompt written against it. Define
	 * LTAZ_MCP_PREFIX in wp-config.php to rebrand (e.g. "ltaz_").
	 */
	const CATEGORY = "Lee's Trees";

	private $prefix;

	// Uploads retention (automatic age-based cleanup of a single uploads subdirectory)
	const RETENTION_OPTION = 'ltaz_uploads_retention';
	const RETENTION_LOG    = 'ltaz_uploads_retention_log';
	const RETENTION_CRON   = 'ltaz_uploads_retention_run';
	const RETENTION_MIN_AGE_FLOOR = 30;   // refuse to treat anything younger than this as expired

	public function __construct() {
		$this->prefix = defined( 'LTAZ_MCP_PREFIX' ) ? (string) LTAZ_MCP_PREFIX : 'pps_';
	...
	/**
	 * The one theme directory the theme_* tools may touch.
	 *
	 * PPS hard-codes its own theme slug; here we default to the active stylesheet
	 * (the child theme when one is active), which is the only theme it is ever safe
	 * to edit in place. Override with LTAZ_MCP_THEME_SLUG in wp-config.php.
	 */
	private function theme_slug() : string {
		if ( defined( 'LTAZ_MCP_THEME_SLUG' ) && LTAZ_MCP_THEME_SLUG ) {
			return (string) LTAZ_MCP_THEME_SLUG;
		}
	...
	private function tools() {
		$tools = array(
			...
			$this->prefix . 'woo_list_products' => array(
				'name'        => $this->prefix . 'woo_list_products',
			...
			$this->prefix . 'woo_get_product' => array(
				'name'        => $this->prefix . 'woo_get_product',
			...
			$this->prefix . 'woo_update_product' => array(
				'name'        => $this->prefix . 'woo_update_product',
			...
			$this->prefix . 'woo_list_categories' => array(
				'name'        => $this->prefix . 'woo_list_categories',
			...
			$this->prefix . 'woo_get_category' => array(
				'name'        => $this->prefix . 'woo_get_category',
			...
			$this->prefix . 'woo_list_orders' => array(
				'name'        => $this->prefix . 'woo_list_orders',
			...
			$this->prefix . 'woo_get_order' => array(
				'name'        => $this->prefix . 'woo_get_order',
			...
			$this->prefix . 'woo_update_order_status' => array(
				'name'        => $this->prefix . 'woo_update_order_status',
			...
			$this->prefix . 'woo_add_order_note' => array(
				'name'        => $this->prefix . 'woo_add_order_note',
			...
			$this->prefix . 'theme_list_files' => array(
				'name'        => $this->prefix . 'theme_list_files',
			...
			$this->prefix . 'theme_read_file' => array(
				'name'        => $this->prefix . 'theme_read_file',
			...
			$this->prefix . 'theme_write_file' => array(
				'name'        => $this->prefix . 'theme_write_file',
			...
			$this->prefix . 'plugin_list_files' => array(
				'name'        => $this->prefix . 'plugin_list_files',
			...
			$this->prefix . 'plugin_read_file' => array(
				'name'        => $this->prefix . 'plugin_read_file',
			...
			$this->prefix . 'plugin_write_file' => array(
				'name'        => $this->prefix . 'plugin_write_file',
			...
			$this->prefix . 'plugin_download_url' => array(
				'name'        => $this->prefix . 'plugin_download_url',
			...
			$this->prefix . 'plugin_delete_file' => array(
				'name'        => $this->prefix . 'plugin_delete_file',
			...
			$this->prefix . 'uploads_list_files' => array(
				'name'        => $this->prefix . 'uploads_list_files',
			...
			$this->prefix . 'uploads_delete_file' => array(
				'name'        => $this->prefix . 'uploads_delete_file',
			...
			$this->prefix . 'uploads_delete_batch' => array(
				'name'        => $this->prefix . 'uploads_delete_batch',
			...
			$this->prefix . 'uploads_retention_get' => array(
				'name'        => $this->prefix . 'uploads_retention_get',
			...
			$this->prefix . 'uploads_retention_set' => array(
				'name'        => $this->prefix . 'uploads_retention_set',
			...
			$this->prefix . 'uploads_retention_run_now' => array(
				'name'        => $this->prefix . 'uploads_retention_run_now',
			...
			$this->prefix . 'wp_check_updates' => array(
				'name'        => $this->prefix . 'wp_check_updates',
				'description' => 'Refresh and return available core, plugin, and theme updates. Read-only.',
				'inputSchema' => array( 'type' => 'object', 'properties' => (object) array() ),
			),

			$this->prefix . 'wp_get_plugin_versions' => array(
				'name'        => $this->prefix . 'wp_get_plugin_versions',
				'description' => 'List all installed plugins with current version, active status, and upgrade-target version.',
				'inputSchema' => array( 'type' => 'object', 'properties' => (object) array() ),
			),

			$this->prefix . 'wp_update_plugin' => array(
				'name'        => $this->prefix . 'wp_update_plugin',
			...
			$this->prefix . 'wp_update_theme' => array(
				'name'        => $this->prefix . 'wp_update_theme',
			...
			$this->prefix . 'wp_update_core' => array(
				'name'        => $this->prefix . 'wp_update_core',
				'description' => 'WRITE OPERATION. Update WordPress core via Core_Upgrader.',
				'inputSchema' => array( 'type' => 'object', 'properties' => (object) array() ),
			),
		);

		// WooCommerce is not installed on every site this plugin targets. Advertising a
		// tool that can only ever answer "WooCommerce is not active" is worse than not
		// advertising it, so the woo_* group is dropped when Woo is absent.
		if ( ! class_exists( 'WooCommerce' ) ) {
			foreach ( array_keys( $tools ) as $name ) {
				if ( strpos( $name, $this->prefix . 'woo_' ) === 0 ) {
					unset( $tools[ $name ] );
				}
			}
		}

		return array_merge( $tools, $this->site_tools() );
	}
	...
	public function register_tools( array $prev ) : array {
		...
		foreach ( $tools as &$tool ) {
			...
			$is_readonly = (
				strpos( $name, $this->prefix . 'list_'      ) === 0 ||
				strpos( $name, $this->prefix . 'get_'       ) === 0 ||
				strpos( $name, $this->prefix . 'woo_list_'  ) === 0 ||
				strpos( $name, $this->prefix . 'woo_get_'   ) === 0 ||
				$name === $this->prefix . 'theme_list_files'  ||
				$name === $this->prefix . 'theme_read_file'   ||
				$name === $this->prefix . 'plugin_list_files' ||
				$name === $this->prefix . 'plugin_read_file'  ||
			...
			$is_destructive = (
				...
				$name === $this->prefix . 'wp_update_theme'         ||
				$name === $this->prefix . 'wp_update_core'         ||
				$name === $this->prefix . 'delete_menu_item'       ||
				$name === $this->prefix . 'delete_reusable_block'  ||
				$name === $this->prefix . 'set_theme_mod'          ||
				$name === $this->prefix . 'set_astra_setting'      ||
				$name === $this->prefix . 'set_custom_css'         ||
				$name === $this->prefix . 'set_site_identity'
			);

			// JSON Schema requires "properties" to be an object. An empty PHP array
			// encodes as [] and MCP validators reject it, so cast it here rather than
			// relying on every tool definition remembering the (object) cast.
			if ( isset( $tool['inputSchema']['properties'] )
				&& is_array( $tool['inputSchema']['properties'] )
				&& empty( $tool['inputSchema']['properties'] ) ) {
				$tool['inputSchema']['properties'] = (object) array();
			}

			$tool['annotations'] = array(
				'readOnlyHint'    => $is_readonly,
				'destructiveHint' => $is_destructive,
				'openWorldHint'   => $name === $this->prefix . 'plugin_download_url',
			);
		}
		unset( $tool );
	...
	public function handle_call( $prev, string $tool, array $args, ?int $id ) {
		...
		try {
			switch ( $tool ) {
				case $this->prefix . 'woo_list_products':       $data = $this->woo_list_products( $args ); break;
				case $this->prefix . 'woo_get_product':         $data = $this->woo_get_product( $args ); break;
				case $this->prefix . 'woo_update_product':      $data = $this->woo_update_product( $args ); break;
				case $this->prefix . 'woo_list_categories':     $data = $this->woo_list_categories(); break;
				case $this->prefix . 'woo_get_category':        $data = $this->woo_get_category( $args ); break;
				case $this->prefix . 'woo_list_orders':         $data = $this->woo_list_orders( $args ); break;
				case $this->prefix . 'woo_get_order':           $data = $this->woo_get_order( $args ); break;
				case $this->prefix . 'woo_update_order_status': $data = $this->woo_update_order_status( $args ); break;
				case $this->prefix . 'woo_add_order_note':      $data = $this->woo_add_order_note( $args ); break;
				case $this->prefix . 'theme_list_files':        $data = $this->theme_list_files( $args ); break;
				case $this->prefix . 'theme_read_file':         $data = $this->theme_read_file( $args ); break;
				case $this->prefix . 'theme_write_file':        $data = $this->theme_write_file( $args ); break;
				case $this->prefix . 'plugin_list_files':       $data = $this->plugin_list_files( $args ); break;
				case $this->prefix . 'plugin_read_file':        $data = $this->plugin_read_file( $args ); break;
				case $this->prefix . 'plugin_write_file':       $data = $this->plugin_write_file( $args ); break;
				case $this->prefix . 'plugin_download_url':     $data = $this->plugin_download_url( $args ); break;
				case $this->prefix . 'plugin_delete_file':      $data = $this->plugin_delete_file( $args ); break;
				case $this->prefix . 'uploads_list_files':      $data = $this->uploads_list_files( $args ); break;
				case $this->prefix . 'uploads_delete_file':     $data = $this->uploads_delete_file( $args ); break;
				case $this->prefix . 'uploads_delete_batch':    $data = $this->uploads_delete_batch( $args ); break;
				case $this->prefix . 'uploads_retention_get':     $data = $this->uploads_retention_get(); break;
				case $this->prefix . 'uploads_retention_set':     $data = $this->uploads_retention_set( $args ); break;
				case $this->prefix . 'uploads_retention_run_now': $data = $this->uploads_retention_run_now( $args ); break;
				case $this->prefix . 'wp_check_updates':        $data = $this->wp_check_updates(); break;
				case $this->prefix . 'wp_get_plugin_versions':  $data = $this->wp_get_plugin_versions(); break;
				case $this->prefix . 'wp_update_plugin':        $data = $this->wp_update_plugin( $args ); break;
				case $this->prefix . 'wp_update_theme':         $data = $this->wp_update_theme( $args ); break;
				case $this->prefix . 'wp_update_core':          $data = $this->wp_update_core(); break;

				case $this->prefix . 'list_nav_menus':          $data = $this->list_nav_menus(); break;
				case $this->prefix . 'get_nav_menu':            $data = $this->get_nav_menu( $args ); break;
				case $this->prefix . 'create_nav_menu':         $data = $this->create_nav_menu( $args ); break;
				case $this->prefix . 'save_menu_item':          $data = $this->save_menu_item( $args ); break;
				case $this->prefix . 'delete_menu_item':        $data = $this->delete_menu_item( $args ); break;
				case $this->prefix . 'assign_menu_location':    $data = $this->assign_menu_location( $args ); break;
				case $this->prefix . 'list_reusable_blocks':    $data = $this->list_reusable_blocks( $args ); break;
				case $this->prefix . 'get_reusable_block':      $data = $this->get_reusable_block( $args ); break;
				case $this->prefix . 'save_reusable_block':     $data = $this->save_reusable_block( $args ); break;
				case $this->prefix . 'delete_reusable_block':   $data = $this->delete_reusable_block( $args ); break;
				case $this->prefix . 'get_theme_mods':          $data = $this->get_theme_mods_tool( $args ); break;
				case $this->prefix . 'set_theme_mod':           $data = $this->set_theme_mod_tool( $args ); break;
				case $this->prefix . 'get_astra_setting':       $data = $this->get_astra_setting( $args ); break;
				case $this->prefix . 'set_astra_setting':       $data = $this->set_astra_setting( $args ); break;
				case $this->prefix . 'get_custom_css':          $data = $this->get_custom_css( $args ); break;
				case $this->prefix . 'set_custom_css':          $data = $this->set_custom_css( $args ); break;
				case $this->prefix . 'get_site_identity':       $data = $this->get_site_identity(); break;
				case $this->prefix . 'set_site_identity':       $data = $this->set_site_identity( $args ); break;
				case $this->prefix . 'list_sidebars':           $data = $this->list_sidebars(); break;
				case $this->prefix . 'get_sidebars_widgets':    $data = $this->get_sidebars_widgets_tool(); break;
	...
	private function safe_theme_path( string $relative_path ) : string {
		$theme_root = trailingslashit( get_theme_root() ) . $this->theme_slug();
		$real_root  = realpath( $theme_root );
		if ( ! $real_root ) throw new Exception( 'Theme directory not found: ' . $this->theme_slug() );
		...
		if ( ! $check ) throw new Exception( 'Path is outside the allowed theme directory.' );
		if ( $check !== $real_root && strpos( $check, $real_root . DIRECTORY_SEPARATOR ) !== 0 ) {
	...
	private function safe_plugin_path( string $relative_path ) : string {
		...
		$check = file_exists( $full_path ) ? realpath( $full_path ) : realpath( dirname( $full_path ) );
		if ( ! $check ) throw new Exception( 'Path is outside the plugins directory.' );
		if ( $check !== $plugins_root && strpos( $check, $plugins_root . DIRECTORY_SEPARATOR ) !== 0 ) {
	 ...
	 *
	 * Schemas mirror the PPS staging surface exactly (same tool names, same
	 * argument names) so anything written against that server works here
	 * unchanged. Handlers are written against WordPress core APIs.
	 * ================================================================= */

	private function site_tools() : array {
		$p = $this->prefix;

		$tools = array(

			$p . 'list_nav_menus' => array(
				'name'        => $p . 'list_nav_menus',
			...
			$p . 'create_nav_menu' => array(
				'name'        => $p . 'create_nav_menu',
			...
			$p . 'save_menu_item' => array(
				'name'        => $p . 'save_menu_item',
			...
			$p . 'delete_menu_item' => array(
				'name'        => $p . 'delete_menu_item',
			...
			$p . 'assign_menu_location' => array(
				'name'        => $p . 'assign_menu_location',
			...
			$p . 'list_reusable_blocks' => array(
				'name'        => $p . 'list_reusable_blocks',
			...
			$p . 'get_reusable_block' => array(
				'name'        => $p . 'get_reusable_block',
			...
			$p . 'save_reusable_block' => array(
				'name'        => $p . 'save_reusable_block',
			...
			$p . 'delete_reusable_block' => array(
				'name'        => $p . 'delete_reusable_block',
			...
			$p . 'get_theme_mods' => array(
				'name'        => $p . 'get_theme_mods',
			...
			$p . 'set_theme_mod' => array(
				'name'        => $p . 'set_theme_mod',
				...
				'inputSchema' => array(
					...
					'properties' => array(
						...
						'value'      => array( 'description' => 'Value of any JSON type (string, number, boolean, array, object).' ),
						'stylesheet' => array( 'type' => 'string', 'description' => 'Optional theme folder slug. Defaults to the active theme.' ),
					),
			...
			$p . 'get_astra_setting' => array(
				'name'        => $p . 'get_astra_setting',
			...
			$p . 'set_astra_setting' => array(
				'name'        => $p . 'set_astra_setting',
			...
			$p . 'get_custom_css' => array(
				'name'        => $p . 'get_custom_css',
			...
			$p . 'set_custom_css' => array(
				'name'        => $p . 'set_custom_css',
			...
			$p . 'get_site_identity' => array(
				'name'        => $p . 'get_site_identity',
				'description' => 'Get site title, tagline, custom logo, and site icon (favicon).',
				'inputSchema' => array( 'type' => 'object', 'properties' => (object) array() ),
			),

			$p . 'set_site_identity' => array(
				'name'        => $p . 'set_site_identity',
			...
			$p . 'list_sidebars' => array(
				'name'        => $p . 'list_sidebars',
				'description' => 'List registered sidebars/widget areas and the widget ids currently in each. Read-only.',
				'inputSchema' => array( 'type' => 'object', 'properties' => (object) array() ),
			),

			$p . 'get_sidebars_widgets' => array(
				'name'        => $p . 'get_sidebars_widgets',
				'description' => 'Return the raw sidebars-to-widgets assignment map (wp_get_sidebars_widgets). Read-only.',
				'inputSchema' => array( 'type' => 'object', 'properties' => (object) array() ),
			),
		);

		// The Astra tools read and write the astra-settings option, which only exists
		// when Astra is the active theme family. Same rule as the woo_* group.
		if ( ! $this->astra_active() ) {
			unset( $tools[ $p . 'get_astra_setting' ], $tools[ $p . 'set_astra_setting' ] );
		}

		return $tools;
	}

	private function astra_active() : bool {
		$theme = wp_get_theme();
		return in_array( 'astra', array( $theme->get_stylesheet(), $theme->get_template() ), true )
			|| get_option( 'astra-settings' ) !== false;
	}
	...
	private function resolve_menu( $menu ) {
		...
		if ( ! $obj ) {
			throw new Exception( 'Menu not found: ' . (string) $menu );
		}
		return $obj;
	}

	private function list_nav_menus() : array {
		$locations = get_nav_menu_locations();

		$by_menu = array();
		foreach ( $locations as $loc => $menu_id ) {
			$by_menu[ (int) $menu_id ][] = $loc;
		}
		...
		foreach ( wp_get_nav_menus() as $menu ) {
			$menus[] = array(
				...
				'locations' => isset( $by_menu[ (int) $menu->term_id ] ) ? $by_menu[ (int) $menu->term_id ] : array(),
			);
		}
	...
	private function get_nav_menu( array $args ) : array {
		$menu  = $this->resolve_menu( isset( $args['menu'] ) ? $args['menu'] : null );
		...
		return array(
			'menu'  => array( 'term_id' => (int) $menu->term_id, 'name' => $menu->name, 'slug' => $menu->slug ),
			'count' => count( $out ),
			'items' => $out,
		);
	}
	...
	private function save_menu_item( array $args ) : array {
		...
		if ( $item_id > 0 ) {
			...
			if ( ! $existing || $existing->post_type !== 'nav_menu_item' ) {
				throw new Exception( 'Not a menu item: ' . $item_id );
			}
		}

		$type = isset( $args['item_type'] ) ? sanitize_text_field( $args['item_type'] ) : 'post_type';

		// menu-item-status must be publish or the item is saved as a draft and
		// never renders in the menu.
		$data = array(
			'menu-item-type'   => $type,
			'menu-item-status' => 'publish',
		);

		if ( isset( $args['title'] ) )     { $data['menu-item-title']     = sanitize_text_field( $args['title'] ); }
		if ( isset( $args['url'] ) )       { $data['menu-item-url']       = esc_url_raw( $args['url'] ); }
		if ( isset( $args['parent'] ) )    { $data['menu-item-parent-id'] = (int) $args['parent']; }
		if ( isset( $args['position'] ) )  { $data['menu-item-position']  = (int) $args['position']; }

		if ( $type === 'custom' ) {
			if ( $item_id === 0 && empty( $data['menu-item-url'] ) ) {
				throw new Exception( 'A custom-link item requires url.' );
			}
		}
		else {
			if ( isset( $args['object'] ) )    { $data['menu-item-object']    = sanitize_text_field( $args['object'] ); }
			if ( isset( $args['object_id'] ) ) { $data['menu-item-object-id'] = (int) $args['object_id']; }
			if ( $item_id === 0 && empty( $data['menu-item-object-id'] ) ) {
				throw new Exception( 'Linking a ' . $type . ' item requires object_id (and object, e.g. "page").' );
			}
		}
		...
		return array(
			...
			'created'      => $item_id === 0,
			'menu'         => array( 'term_id' => (int) $menu->term_id, 'name' => $menu->name ),
		);
	}

	private function delete_menu_item( array $args ) : array {
		$id = isset( $args['menu_item_id'] ) ? (int) $args['menu_item_id'] : 0;
		if ( $id <= 0 ) {
			throw new Exception( 'menu_item_id is required.' );
		}
		$post = get_post( $id );
		if ( ! $post || $post->post_type !== 'nav_menu_item' ) {
			throw new Exception( 'Not a menu item: ' . $id );
		}
		if ( ! wp_delete_post( $id, true ) ) {
			throw new Exception( 'Could not delete menu item ' . $id );
		}
		return array( 'success' => true, 'menu_item_id' => $id );
	}
	...
	private function assign_menu_location( array $args ) : array {
		...
		if ( ! isset( $registered[ $location ] ) ) {
			throw new Exception(
				'Unknown theme location: ' . $location .
				'. Registered locations: ' . ( $registered ? implode( ', ', array_keys( $registered ) ) : '(none)' )
			);
		}

		$raw       = isset( $args['menu'] ) ? $args['menu'] : null;
		$locations = get_nav_menu_locations();
	...
	private function require_reusable_block( int $id ) {
		if ( $id <= 0 ) {
	...
	private function get_reusable_block( array $args ) : array {
		$post = $this->require_reusable_block( isset( $args['id'] ) ? (int) $args['id'] : 0 );
		return array(
			'id'       => (int) $post->ID,
			'title'    => $post->post_title,
			'slug'     => $post->post_name,
	...
	private function save_reusable_block( array $args ) : array {
		...
		$postarr = array( 'post_type' => 'wp_block' );
		...
		if ( $id > 0 ) {
			$this->require_reusable_block( $id );
			$postarr['ID'] = $id;
		...
		return array( 'success' => true, 'id' => (int) $result, 'created' => $id === 0 );
	}
	...
	private function delete_reusable_block( array $args ) : array {
		...
		if ( ! wp_delete_post( $id, $force ) ) {
			throw new Exception( 'Could not delete reusable block ' . $id );
		}
		return array( 'success' => true, 'id' => $id, 'forced' => $force );
	}

	/* ---------- theme mods / Astra / custom CSS ---------- */

	private function resolve_stylesheet( array $args ) : string {
		if ( empty( $args['stylesheet'] ) ) {
			return get_stylesheet();
		}
		$slug = sanitize_text_field( $args['stylesheet'] );
		if ( strpos( $slug, '/' ) !== false || strpos( $slug, '\\' ) !== false || strpos( $slug, '..' ) !== false ) {
			throw new Exception( 'Invalid stylesheet slug.' );
		}
		return $slug;
	}

	private function get_theme_mods_tool( array $args ) : array {
		$slug = $this->resolve_stylesheet( $args );
	...
	private function set_theme_mod_tool( array $args ) : array {
		...
		$previous = array_key_exists( $key, $mods ) ? $mods[ $key ] : null;

		// set_theme_mod() fires the pre_set_theme_mod_* filters and handles caching,
		// but only ever writes the ACTIVE theme. For any other stylesheet the option
		// is the only route.
	...
	private function get_astra_setting( array $args ) : array {
		...
		if ( isset( $args['key'] ) && $args['key'] !== '' ) {
			...
			if ( ! array_key_exists( $key, $settings ) ) {
				throw new Exception( 'astra-settings key not found: ' . $key );
			}
			return array( 'key' => $key, 'value' => $settings[ $key ] );
		}
		return array( 'count' => count( $settings ), 'astra_settings' => $settings );
	}
	...
	private function set_astra_setting( array $args ) : array {
		...
		$previous          = array_key_exists( $key, $settings ) ? $settings[ $key ] : null;
		$settings[ $key ]  = $args['value'];   // read-modify-write: every other key survives
		update_option( 'astra-settings', $settings );

		return array( 'success' => true, 'key' => $key, 'previous' => $previous, 'value' => $args['value'] );
	}

	private function get_custom_css( array $args ) : array {
		$slug = $this->resolve_stylesheet( $args );
		$css  = (string) wp_get_custom_css( $slug );
		return array( 'stylesheet' => $slug, 'length' => strlen( $css ), 'css' => $css );
	}

	private function set_custom_css( array $args ) : array {
		if ( ! isset( $args['css'] ) ) {
			throw new Exception( 'css is required.' );
		}
		$slug   = $this->resolve_stylesheet( $args );
		...
		if ( $append ) {
			$existing = (string) wp_get_custom_css( $slug );
			$css      = $existing === '' ? $css : $existing . "\n" . $css;
		}

		$result = wp_update_custom_css_post( $css, array( 'stylesheet' => $slug ) );
		if ( is_wp_error( $result ) ) {
			throw new Exception( $result->get_error_message() );
		}

		return array( 'success' => true, 'stylesheet' => $slug, 'appended' => $append, 'length' => strlen( $css ) );
	}

	/* ---------- site identity / sidebars ---------- */

	private function assert_attachment( int $id ) {
		$post = get_post( $id );
		if ( ! $post || $post->post_type !== 'attachment' ) {
			throw new Exception( 'Not an attachment id: ' . $id );
		}
	}

	private function get_site_identity() : array {
		...
		return array(
			...
			'custom_logo'     => $logo_id,
			'custom_logo_url' => $logo_id ? wp_get_attachment_url( $logo_id ) : null,
			'site_icon'       => $icon_id,
			'site_icon_url'   => $icon_id ? wp_get_attachment_url( $icon_id ) : null,
		);
	}
	...
	private function set_site_identity( array $args ) : array {
		...
		if ( isset( $args['custom_logo'] ) ) {
			$logo_id = (int) $args['custom_logo'];
			if ( $logo_id > 0 ) {
				$this->assert_attachment( $logo_id );
		...
		if ( isset( $args['site_icon'] ) ) {
			$icon_id = (int) $args['site_icon'];
			if ( $icon_id > 0 ) {
				$this->assert_attachment( $icon_id );
		...
		if ( ! $changed ) {
			throw new Exception( 'Nothing to set: pass at least one of title, tagline, custom_logo, site_icon.' );
		}

		return array( 'success' => true, 'changed' => $changed, 'identity' => $this->get_site_identity() );
	}

	private function list_sidebars() : array {
		...
		foreach ( (array) $wp_registered_sidebars as $id => $sidebar ) {
			$out[] = array(
				...
				'widgets'     => isset( $assignments[ $id ] ) ? array_values( (array) $assignments[ $id ] ) : array(),
			);
		}
	...
	private function get_sidebars_widgets_tool() : array {
